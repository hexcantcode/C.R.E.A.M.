  ...
  constructor() {
    super({
      ...
      void main() { 
        ...
        gl_PointSize = max(vDistance * uBlur * uPointSize, 60.0);
      }`,
      fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        float sparkleValue = sparkleNoise(vInitialPosition, uTime);
        
        vec3 finalColor = texColor.rgb;
        
        float brightness = 0.7 + sparkleValue * 0.3;
        
        float alpha = (1.04 - clamp(vDistance, 0.0, 1.0)) * clamp(smoothstep(-0.5, 0.25, vPosY), 0.0, 1.0) * uOpacity * revealMask * uRevealProgress * brightness * texColor.a * 0.3;

        gl_FragColor = vec4(finalColor, mix(alpha, brightness - 1.1, uTransition));
      }`,
      uniforms: {
